...
//        return dd($activation);
    ...
    {
        try {
            $user->update($request->except('pic_file','password','password_confirm','groups','activate', 'bio', 'gender', 'dob', 'pic', 'country', 'state', 'city', 'address','postal', 'receive_email', 'receive_email_two'));
            if ( !empty($request->password)) {
                $user->password = Hash::make($request->password);
            }
            $setting = Setting::first();
            $setting->receive_email = $request->receive_email;
            $setting->receive_email_two = $request->receive_email_two;
            $setting->save();
            $user->save();
